...
export let tuesmonContribPlugins = [];
...
(window as any).tuesmonContribPlugins = [];
...
function loadPlugin(pluginPath: string) {
    return new Promise(function(resolve, reject) {
        const success = function(plugin) {
            (window as any).tuesmonContribPlugins.push(plugin);
            if (plugin.css) {
...
promise.done((data) => (window as any).tuesmonConfig = _.assign({}, (window as any).tuesmonConfig, data));
promise.fail(() => console.error("Your conf.json file is not a valid json file, please review it."));
promise.always(function() {
    if (!(window as any).tuesmonConfig.debug) {
        enableProdMode();
    }

    if ((window as any).tuesmonConfig.contribPlugins.length > 0) {
        return loadPlugins((window as any).tuesmonConfig.contribPlugins).then(() => {
