...
function backup() {
    child_process.spawnSync('pg_dump', ['-c', '-d', 'tuesmon', '-f', 'tmp/tuesmon.sql'], {stdio: "inherit"});
}

function launchProtractor(suit) {
    let protractorParams = ['conf.e2e.js', '--suite=' + suit, '--back=' + tuesmonBackPath];
...
function restoreBackup() {
    child_process.spawnSync('psql', ['-d', 'tuesmon', '-f', 'tmp/tuesmon.sql']);
}
    ...
    inquirer.prompt([
        {
            type: 'string',
            name: 'back',
            message: 'Tuesmon back path'
        }
    ], function (answer) {
        tuesmonBackPath = answer.back;
        launch();
    });
}
